...
case package
  -> inventory check
  -> upload staging and guardrails when reviewer supplies ad hoc files
  ...
  -> CLI / Streamlit case queue and review cockpit
  -> JSON / trace / Markdown / XLSX triage workbook exports
```
...
The app now starts with a procurement-style case queue instead of opening directly on one sample case. The queue shows case ID, vendor, status, risk, ACV, TCV, budget, missing-info count, blocker count, and next owner.
