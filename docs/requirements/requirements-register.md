...
| REQ-018 | Uploaded vendor package mode | Reviewer testing risk | Must | App accepts a new single-vendor package as multiple files or a zip, validates required artifacts, stages files as a temporary standalone case, does not mutate sample cases, recognizes optional support artifacts, shows package delta, and runs the same deterministic pipeline. | Upload unit tests plus Streamlit smoke test | Verified |
...
| REQ-023 | Productized reviewer UX | Exam: product judgment; Communication Policy; process-flow PNG | Must | Primary UI avoids implementation-mode sidebar copy and fake controls; case review starts with an action cockpit, next owner, required vendor follow-up, internal route, and editable AI-assisted drafts; detailed evidence, workflow, trace, and exports stay behind audit disclosure. | Streamlit regression test plus browser UX audit | Verified |
...
- Upload mode is required for reviewer-driven testing, but it should not fork the business logic. Uploaded packages must stage into the same canonical case structure, call the same pipeline as sample cases, and make clear that uploaded files create a temporary triage case instead of modifying bundled samples.
...
- `REQ-018` is verified for multi-file uploads, zip uploads, optional support artifacts, package delta, temporary-case copy, and missing-file validation.
