...
| REQ-010 | Streamlit review cockpit | Exam: working prototype usable by others | Must | App lets reviewer start from a vendor request queue, open request details, inspect decision summary, findings, evidence, drafts, workflow, and trace, and export outputs. | Streamlit smoke test plus manual UX checklist | Verified |
...
| REQ-014 | Deployment path | Exam: deployed environment usable by others | Must | Public deployed Streamlit app can run all cases with no local setup. | Deployed smoke test | Verified |
...
| REQ-016 | Public sharing and secrets | Submission sharing | Must | Repo contains only synthetic exercise materials and no secrets; `.env.example` documents env vars only. | Git status and secret scan/manual review | Verified |
...
| REQ-018 | Uploaded vendor package mode | Reviewer testing risk | Must | App accepts a new single-vendor package as multiple files or a zip, validates required artifacts, stages files into a new request record, does not mutate seeded requests, recognizes optional support artifacts, and runs the same deterministic pipeline. | Upload unit tests plus Streamlit smoke test | Verified |
| REQ-019 | Vendor request queue dashboard | Product UX and procurement workflow | Must | App home shows all seeded and session-uploaded vendor requests with status, risk, spend, budget, missing-info count, next owner, source, and open action before drilling into a request. | Streamlit regression test plus browser review | Verified |
...
| REQ-022 | Reviewer upload sample packets | QA/product demonstration | Must | Repo includes realistic upload packet folders and zips for valid low-risk, high-risk support-doc, net-new support SaaS, prompt-injection, decoy policy-doc, mixed-vendor, and malformed quote scenarios. | Sample packet regression tests plus README walkthrough | Verified |
| REQ-023 | Productized reviewer UX | Exam: product judgment; Communication Policy; process-flow PNG | Must | Primary UI avoids implementation-mode sidebar copy and fake controls; request review starts with decision, next owner, required vendor follow-up, internal route, and reviewer brief; drafts, upload intake mapping, detailed evidence, workflow, trace, and exports stay behind disclosure. | Streamlit regression test plus browser UX audit | Verified |
| REQ-024 | Optional OpenAI reviewer synthesis | Product UX; process-flow prepare-output stage | Should | System creates a reviewer brief and draft text from the validated decision packet, optionally calls OpenAI with structured output, validates citations and prohibited language, and falls back to deterministic synthesis without changing policy decisions. | Synthesis unit tests, live smoke, Streamlit/workbook checks | Verified |
| REQ-025 | Session request lifecycle | Product UX and demo realism | Must | Seeded exam cases appear as existing vendor requests; uploaded packages are remembered as new session request records; reviewers can open and delete request records; request details keep drafts, upload intake mapping, evidence, trace, and exports behind disclosure. | Streamlit regression test plus browser smoke | Verified |
...
- Upload mode is required for reviewer-driven testing, but it should not fork the business logic. Uploaded packages must stage into the same canonical case structure, call the same pipeline as seeded requests, and create a new request record in the queue.
...
- `REQ-018` is verified for multi-file uploads, zip uploads, optional support artifacts, request-record creation, and missing-file validation.
...
- `REQ-025` is verified by `tests/test_streamlit_app.py` and the request-queue QA note.
