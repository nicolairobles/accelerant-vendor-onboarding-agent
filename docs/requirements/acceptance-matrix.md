...
| REQ-018 | Triage new package tests | `tests/test_upload_mode.py`, `tests/test_streamlit_app.py`, Streamlit smoke test | Multiple files and zip upload stage to a temporary canonical case format; sample cases are not mutated; package delta explains matched baseline or net-new status; missing required files are reported before triage; optional support docs are mapped. | Verified |
...
| REQ-023 | Productized reviewer UX test | `tests/test_streamlit_app.py`, `docs/quality/productization-ux-audit.md` | Sidebar has no implementation-mode copy; missing-info follow-up is not rendered as inert checkboxes; dashboard uses queue-wide Open Requests; case view starts with Action Cockpit, Required Vendor Follow-up, Internal Review Route, and AI-Assisted Drafts; evidence, workflow, trace, and exports are under Audit details. | Verified |
